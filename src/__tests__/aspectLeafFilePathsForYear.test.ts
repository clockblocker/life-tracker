...
import { aspectLeafFilePathsForYear } from '../utils/file-utils';
import { describe, it, expect } from 'vitest';

describe('aspectLeafFilePathsForYear', () => {
	it('returns empty array for no aspects', () => {
		const result = aspectLeafFilePathsForYear(2024, [], [5, 15]);
		expect(result).toEqual([]);
	});

	it('generates one file per aspect × stats × month for empty cutoff', () => {
		const aspects: Aspect[] = ['Sport'];
		const result = aspectLeafFilePathsForYear(2024, aspects, []);
