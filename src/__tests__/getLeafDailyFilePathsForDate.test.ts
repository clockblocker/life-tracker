...
import { getLeafDailyFilePathsForDate } from '../utils/paths';
import { describe, it, expect } from 'vitest';

const utc = (y: number, m: number, d: number) => new Date(Date.UTC(y, m, d));

describe('getLeafDailyFilePathsForDate', () => {
	it('includes only Root and Notes files when aspects are empty', () => {
		const date = utc(2024, 6, 10); // 2024-07-10
		const result = getLeafDailyFilePathsForDate(date, []);
	...
	it('includes all aspect files when all are provided', () => {
		...
		const result = getLeafDailyFilePathsForDate(date, aspects);
		expect(result).toEqual([
	...
	it('handles subset of aspects', () => {
		...
		const result = getLeafDailyFilePathsForDate(date, ['Food']);
		expect(result).toEqual([
	...
	it('outputs same result regardless of aspect order', () => {
		...
		const resultA = getLeafDailyFilePathsForDate(date, ['Food', 'Money']);
		const resultB = getLeafDailyFilePathsForDate(date, ['Money', 'Food']);
