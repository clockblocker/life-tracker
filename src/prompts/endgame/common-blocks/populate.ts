import { TFile, Vault, MetadataCache, Editor, normalizePath, Notice } from "obsidian";
import { doesFileContainContent, appendToFile } from "utils";
import { Backlink } from "../zod/types";

export async function addLinksToRelatedToBlock(vault: Vault, filePath: string, links: string[]): Promise<void> {
    ...
    const formattedTags = links.map(link => link.startsWith("#") ? link : `#${link}`);
    
    const tagBlockRegex = /^(\*Tags\*:\s*)(.*)$/m;
    
    if (tagBlockRegex.test(content)) {
      content = content.replace(tagBlockRegex, (match, prefix, existingTags) => {
        const existingTagsArr = existingTags.split(/\s+/).filter((t: string) => t.trim() !== "");
...
export async function addTagsToTagBlock(vault: Vault, file: TFile, tags: string[]): Promise<void> {
    let content = await vault.read(file);
...
export default async function populateBacklinks(vault: Vault, filePath: string, metadataCache: MetadataCache, editor: Editor, backlinks: Backlink[]) {
    try {
        const file = vault.getAbstractFileByPath(filePath);
        if (!file || !(file instanceof TFile)) {
          console.error("File not found or is not a valid file");
