...
function BUILD_oldBlockContentFromId_FROM_fileContent(
  ...
  const oldBlockContentRecord: Record<BlockId, BlockContent> = {} as Record<BlockId, BlockContent>;
  
  ALL_BLOCK_IDS.forEach((blockId) => {
    oldBlockContentRecord[blockId] = extractBlockContent({ content: fileContent, blockId });
  });
...
export async function makeNewFileContent({
  ...
  const oldBlockContentFromId = BUILD_oldBlockContentFromId_FROM_fileContent(oldFileContent);
  const blockContentsFromIds = [newBlockContentFromId, oldBlockContentFromId];
  const mergedContentFromBlockId = mergeBlockContentsFromIds({ blockContentsFromIds, setOfBlockIdsToCreateIfEmpty });
